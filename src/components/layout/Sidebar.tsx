...
import { LayoutDashboard, BookOpen, GraduationCap, Trophy, Calendar, MessageSquare, Award, Settings, User, ChevronLeft, ChevronRight, Flame, X, LogOut, Shield } from 'lucide-react';
...
export default function Sidebar({ currentPage, isOpen, onNavigate, onToggle }: SidebarProps) {
  ...
  const bottomItems = [
    ...(user?.role === 'admin' ? [{ id: 'admin' as Page, label: t('nav.admin'), icon: Shield }] : []),
          ...
          {isOpen ? (
            <div className="flex w-full items-center justify-between">
              <div className="flex items-center gap-2.5">
                <img src="/images/logo.svg" alt="LearnHub" className="h-9" />
              </div>
          ...
          <div className={`border-t px-3 py-3 ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
            <button onClick={() => onNavigate('profile')} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-50'}`}>
              <div className="h-9 w-9 overflow-hidden rounded-full">
                <img src={getAvatarUrl(fullName)} alt="" className="h-full w-full object-cover" loading="lazy" />
              </div>
