...
import { ArrowLeft, ArrowRight, Play, Pause, CheckCircle2, BookOpen, ChevronDown, ChevronUp, FileText, HelpCircle, Code, Lock, ThumbsUp, Bookmark, Settings, Volume2, Maximize2, SkipForward } from 'lucide-react';
  ...
  return (
    <div className={`flex h-[calc(100vh-64px)] flex-col lg:flex-row ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="flex flex-1 flex-col overflow-hidden">
          ...
          {currentLesson.type === 'video' ? (
            <div className="relative flex aspect-video max-h-[60vh] items-center justify-center">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <div className="mb-2 h-1 overflow-hidden rounded-full bg-white/20"><div className="h-full w-1/3 rounded-full bg-primary-500" /></div>
        ...
        <div className={`flex items-center justify-between border-b px-4 py-3 md:px-6 ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
          <div className="flex items-center gap-3">
            <button onClick={() => nav('course-detail', courseId || undefined)}
              className={`flex items-center gap-1 text-sm ${isDark ? 'text-slate-400 hover:text-slate-300' : 'text-slate-500 hover:text-slate-700'}`}>
              <ArrowLeft className="h-4 w-4" /><span className="hidden md:inline">{t('lesson.backToCourse')}</span>
            </button>
          ...
          <div className="flex items-center gap-2">
            <button className={`rounded-lg p-2 ${isDark ? 'text-slate-500 hover:bg-slate-700 hover:text-slate-300' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}><Bookmark className="h-4 w-4" /></button>
            <button className={`rounded-lg p-2 ${isDark ? 'text-slate-500 hover:bg-slate-700 hover:text-slate-300' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}><ThumbsUp className="h-4 w-4" /></button>
            <button onClick={() => setShowSidebar(!showSidebar)}
              className={`rounded-lg p-2 lg:hidden ${isDark ? 'text-slate-500 hover:bg-slate-700 hover:text-slate-300' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}><BookOpen className="h-4 w-4" /></button>
          </div>
        </div>
        ...
        <div className={`flex-1 overflow-y-auto p-4 md:p-6 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
          <div className="mx-auto max-w-3xl">
            <div className="mb-6">
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder={t('lesson.takeNotes')}
                className={`h-24 w-full rounded-xl border p-3 text-sm outline-none resize-none ${isDark ? 'border-slate-700 bg-slate-800 text-white placeholder-slate-500 focus:border-primary-500' : 'border-slate-200 bg-white placeholder-slate-400 focus:border-primary-300 focus:ring-2 focus:ring-primary-100'}`} />
            </div>

            <div className="flex items-center justify-between">
              <button onClick={() => prevLesson && nav('lesson', courseId || undefined, prevLesson.id)} disabled={!prevLesson}
                className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium disabled:opacity-40 ${isDark ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-700 hover:bg-slate-50'}`}>
              ...
              <button onClick={() => nextLesson && nav('lesson', courseId || undefined, nextLesson.id)} disabled={!nextLesson}
              ...
              return (
                <div key={module.id}>
                  <button onClick={() => toggleModule(module.id)}
                    ...
                    {isExpanded ? <ChevronUp className={`h-4 w-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} /> : <ChevronDown className={`h-4 w-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />}
                  </button>
                  {isExpanded && (
                    <div>
                      {module.lessons.map((lesson) => {
                        ...
                        return (
                          <button key={lesson.id} onClick={() => !lesson.isLocked && nav('lesson', courseId || undefined, lesson.id)} disabled={lesson.isLocked}
                            className={`flex w-full items-center gap-3 px-4 py-2 text-left text-xs transition-all ${isCurrent ? isDark ? 'bg-primary-900/30 border-l-2 border-primary-500' : 'bg-primary-50 border-l-2 border-primary-500' : lesson.isLocked ? 'opacity-40' : isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}>
                            {lesson.isCompleted ? <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-success-500" /> : lesson.isLocked ? <Lock className={`h-4 w-4 flex-shrink-0 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} /> :
                              <Icon className={`h-4 w-4 flex-shrink-0 ${isCurrent ? 'text-primary-500' : isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                            }
                            <span className={`flex-1 truncate ${isCurrent ? isDark ? 'font-medium text-primary-400' : 'font-medium text-primary-700' : isDark ? 'text-slate-300' : 'text-slate-600'}`}>{lesson.title}</span>
