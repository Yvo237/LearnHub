...
import { courses, forumPosts } from '../../data/mockData';
...
export default function CourseDetail({ courseId, onNavigate }: CourseDetailProps) {
  ...
  const coursePosts = forumPosts.filter(p => p.courseId === course.id);

  const courseColors = ['from-blue-500 to-cyan-500', 'from-purple-500 to-pink-500', 'from-orange-500 to-red-500', 'from-emerald-500 to-teal-500', 'from-indigo-500 to-violet-500'];
  const colorIndex = parseInt(course.id.replace('c', '')) % courseColors.length;

  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      {/* Hero */}
      <div className={`relative bg-gradient-to-br ${courseColors[colorIndex]} px-4 py-6 text-white md:px-6 md:py-10`}>
        <div className="absolute inset-0 bg-black/20" />
        <div className="relative">
          <button
            onClick={() => onNavigate(course.isEnrolled ? 'my-courses' : 'catalog')}
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-2xl">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-4 text-sm">
                ...
                <span className="flex items-center gap-1">
                  <Users className="h-4 w-4" /> {course.studentsCount.toLocaleString()} {t('common.students')}
                </span>
                <span className="flex items-center gap-1">
                  <Clock className="h-4 w-4" /> {course.duration}
                </span>
                <span className="flex items-center gap-1">
                  <BookOpen className="h-4 w-4" /> {totalLessons} {t('common.lessons')}
                </span>
              </div>
            ...
            <div className={`w-full rounded-2xl p-5 shadow-xl lg:w-80 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
              {course.isEnrolled ? (
                <>
                  <div className="mb-4">
                    ...
                    <p className={`mt-1 text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                      {completedLessons}/{totalLessons} {t('course.lessonsCompleted')}
                    </p>
                  </div>
                  <button 
                    onClick={() => {
                      const nextLesson = course.modules.flatMap(m => m.lessons).find(l => !l.isCompleted && !l.isLocked);
                      if (nextLesson) onNavigate('lesson', course.id, nextLesson.id);
                    }}
                    ...
                    <span className="flex items-center justify-center gap-2">
                      <Play className="h-4 w-4" /> {t('course.continueCourse')}
                    </span>
                  </button>
                </>
              ) : (
                <>
                  <div className="mb-4 text-center">
                    {course.isFree ? (
                      <p className="text-2xl font-bold text-emerald-600">{t('common.free')}</p>
                    ) : (
                      <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{course.price}EUR</p>
                    )}
                  </div>
                  ...
                  <button className={`mt-2 flex w-full items-center justify-center gap-2 rounded-xl border py-3 text-sm font-medium ${
              ...
              <div className={`mt-4 space-y-2 border-t pt-4 ${isDark ? 'border-slate-700' : 'border-slate-100'}`}>
                <div className={`flex items-center gap-2 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                  <Award className={`h-4 w-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} /> {t('course.certificate')}
                </div>
                <div className={`flex items-center gap-2 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                  <Clock className={`h-4 w-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} /> {t('course.unlimitedAccess')}
                </div>
                <div className={`flex items-center gap-2 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                  <BarChart3 className={`h-4 w-4 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} /> {t('course.progressTracking')}
                </div>
              </div>
      ...
      <div className={`border-b px-4 md:px-6 ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
        <div className="flex gap-6">
          {(['content', 'overview', 'forum', 'reviews'] as const).map((tab) => (
            ...
            >
              {tab === 'content' && t('course.content')}
              {tab === 'overview' && t('course.overview')}
              {tab === 'forum' && `Forum (${coursePosts.length})`}
              {tab === 'reviews' && t('course.reviews')}
            </button>
          ))}
        </div>
      </div>
              ...
              return (
                <div key={module.id} className={`overflow-hidden rounded-2xl border ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
                  <button
                    ...
                    className={`flex w-full items-center justify-between p-4 text-left ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                          {module.lessons.length} {t('common.lessons')} - {module.duration} - {moduleCompleted}/{module.lessons.length}
                        </p>
                      </div>
                    </div>
                    {isExpanded ? (
                      <ChevronUp className={`h-5 w-5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                    ) : (
                          ...
                          onClickLesson={() => {
                            if (!lesson.isLocked && course.isEnrolled) {
                              if (lesson.type === 'quiz') {
                                ...
                                onNavigate('lesson', course.id, lesson.id);
                              }
                            }
                          }}
                        />
            ...
            <div className="prose prose-sm prose-slate dark:prose-invert">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {[
                  ...
                  'Creer des projets professionnels',
                  'Maitriser les bonnes pratiques',
                  'Resoudre des problemes complexes',
                  'Preparer des entretiens techniques',
                  'Developper un portfolio solide',
                ].map((item, i) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`flex h-9 w-9 items-center justify-center rounded-full text-xs font-semibold ${
                      isDark ? 'bg-primary-900/30 text-primary-400' : 'bg-gradient-to-br from-primary-200 to-accent-200 text-primary-700'
                    }`}>
...
function LessonRow({
  ...
  return (
    <div
      ...
      } ${lesson.isLocked ? 'opacity-50' : `cursor-pointer ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}`}
    >
      {lesson.isCompleted ? (
        <CheckCircle2 className="h-5 w-5 flex-shrink-0 text-success-500" />
      ) : lesson.isLocked ? (
        <Lock className={`h-5 w-5 flex-shrink-0 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} />
      ) : (
        <div className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border-2 ${
          isDark ? 'border-slate-600' : 'border-slate-300'
        }`}>
          <span className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{index}</span>
        </div>
      )}
      <Icon className={`h-4 w-4 flex-shrink-0 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
      <div className="flex-1">
        <p className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{lesson.title}</p>
      </div>
